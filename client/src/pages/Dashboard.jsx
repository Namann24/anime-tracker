...
export default function Dashboard() {
  ...
  useEffect(() => {
    const initDashboard = async () => {
      ...
      try {
        ...
        // Fetch trending for 'Sector Intel'
  ...
  const calculateProgress = (anime) => {
    if (!anime) return 0;
    const totalWatched = anime.seasons.reduce((acc, s) => acc + s.watchedEpisodes.length, 0);
    const totalEps = anime.episodes || anime.seasons.reduce((acc, s) => acc + s.totalEpisodes, 0) || 12; // Default to 12 if unknown
    return Math.min(100, Math.round((totalWatched / totalEps) * 100));
  };

  const getNextEpisode = (anime) => {
    // Simple heuristic: find first season with unwatched eps
  ...
  return (
    <div className="min-h-screen pb-20 overflow-x-hidden">
      {/* HERO SECTION - "CURRENT OBJECTIVE" */}
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="relative z-10 max-w-4xl w-full animate-in fade-in slide-in-from-bottom-10 duration-1000">
              ...
              <div className="absolute -top-32 left-0 space-y-2 pointer-events-none hidden md:block">
                {neuralPulse.map(log => (
                  <div key={log.id} className="text-[10px] font-mono text-red-500/40 animate-in fade-in slide-in-from-left-4">
              ...
              <div className="flex flex-wrap gap-6 items-center">
                <SagaButton
                  variant="primary"
                  size="xl"
                  onClick={() => navigate(`/anime/${activeMission.mal_id}`)}
                  className="shadow-neon-red hover:scale-105"
                >
                ...
                <div className="hidden sm:flex flex-col">
                  ...
                  <div className="w-48 h-1.5 bg-[var(--saga-surface)] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${calculateProgress(activeMission)}%` }}
                    >
          ...
          <div className="relative z-10 animate-in fade-in zoom-in duration-1000">
            ...
            <p className="text-[var(--saga-text-dim)] mb-8 max-w-md">Your command log is empty. Initiate a new saga to populate the command center.</p>
            <SagaButton variant="primary" onClick={() => navigate("/search")}>Discover New Intel</SagaButton>
          </div>
        )}
      </section>

      {/* SECONDARY OPERATIONS - "SIDE MISSIONS" */}
      <section className="max-w-[1600px] mx-auto px-6 md:px-12 relative z-10 -mt-10">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {continueWatching.map(anime => (
              <div key={anime._id} onClick={() => navigate(`/anime/${anime.mal_id}`)} className="group cursor-pointer">
                <div className="relative aspect-[16/10] rounded-2xl overflow-hidden border-2 border-[var(--saga-border)] group-hover:border-red-600/50 transition-all duration-500 shadow-lg group-hover:shadow-neon-red">
                  <img src={anime.image} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="" />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-black/80 to-transparent">
                    <h3 className="text-white font-bold text-lg leading-tight line-clamp-1">{anime.title}</h3>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="p-12 border-2 border-dashed border-[var(--saga-border)] rounded-3xl text-center bg-[var(--saga-surface)]">
            <p className="text-[var(--saga-text-dim)] font-bold text-sm">No secondary missions. Focus on the primary objective.</p>
          </div>
        )}
      </section>
      ...
      <section className="max-w-[1600px] mx-auto px-6 md:px-12 mt-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <TerminalButton
            ...
            icon="🎲"
            onClick={() => navigate("/search?sort=random")}
            color="text-purple-500"
          />
          <TerminalButton
            title="System Settings"
            subtitle="Configure UI"
            icon="⚙️"
            onClick={() => navigate("/profile/" + user?.username)}
            color="text-[var(--saga-text-dim)]"
          />
        </div>
      </section>
    </div>
...
function TerminalButton({ title, subtitle, icon, onClick, color }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-4 p-6 rounded-2xl border border-[var(--saga-border)] bg-[var(--saga-surface)] hover:border-[var(--saga-accent)]/50 transition-all group text-left"
    >
      <div className={`text-3xl grayscale group-hover:grayscale-0 transition-all ${color}`}>{icon}</div>
      <div>
        <div className="text-[10px] font-black uppercase tracking-widest text-[var(--saga-text-dim)] group-hover:text-[var(--saga-accent)] transition-colors">{subtitle}</div>
